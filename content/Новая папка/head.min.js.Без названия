...
}function trackSubmitJoin(){utagTrackFormLinkCall("Registration : Header","Registration",1)
...
setTimeout(function(){var b=Math.floor(Math.random()*10000000);
$('*[data-view-tracking="true"]').each(function(){var h="track_"+ ++b;
var e=$(this);
e.attr("id",h);
var d={id:h,elm:e,type:e.attr("data-activity-type")?e.attr("data-activity-type"):"",code:e.attr("data-activity-code")?e.attr("data-activity-code"):"",creative:e.attr("data-promotion-creative")?e.attr("data-promotion-creative"):"",position:e.attr("data-promotion-position")?e.attr("data-promotion-position"):"",top:e.offset().top,left:e.offset().left,width:e.width(),height:e.height(),tracked:false,viewTrackingEnabled:e.attr("data-newarrivals-first-article")?e.attr("data-newarrivals-first-article")==="true":true};
...
a.checkInterval=setInterval(function(){var s=document.documentElement,g=(window.pageXOffset||s.scrollLeft)-(s.clientLeft||0),q=(window.pageYOffset||s.scrollTop)-(s.clientTop||0),f=$(window).height(),m=$(window).width(),j,n=[],b=[],t=[],l=[];
var d=sessionStorage.getItem("debugTracking"),p=window.debugTracking||d;
if(p==true){if(d!=true){try{sessionStorage.setItem("debugTracking",p)
}catch(h){}}}for(var k=0;
k<a.trackedElementsInPage.length;
++k){var o=a.trackedElementsInPage[k];
if(o.viewTrackingEnabled){o.top=o.elm.offset().top;
o.left=o.elm.offset().left;
o.width=o.elm.width();
o.height=o.elm.height();
if(p){if(o.tracked){$("#"+o.id).css("border","3px solid red")
}else{$("#"+o.id).css("border","3px solid blue")
}}if(o.height>0&&o.width>0&&q>=q&&(o.top+o.height)<=q+f&&o.left>=g&&(o.left+o.width)<=g+m){if(!o.tracked){j=o.tracked=true;
n.push(o.type);
b.push(o.code);
t.push(o.creative);
l.push(o.position)
}}}}if(j){var c="area_visible";
if(a.isViewClick){c="area_visible_click";
a.isViewClick=false
}var r={event_type:c,event_category:"Internal Promotions",event_id:"view",promotion_name:n,promotion_id:b,promotion_creative:t,promotion_position:l};
if(window.utag!=undefined){window.utag.link(r);
if(p){console.log("viewtrack utag.link sent with payload:");
console.log(r)
}}else{console.log("utag is undefined")
}}},500)
})(promotionTracking);
...
var d;
if(e.data("tracking-json-template")){d=$("#"+e.data("tracking-json-template")).text();
var f=e.data("tracking-params").split("|");
$.each(f,function(h){d=d.replace("{"+h+"}",this)
})
}else{d=e.data("tracking-params")
}var g=JSON.parse(d.trim());
if(e.data("tracking-type")=="link"){e.click(function(){if(c){console.log("will be sent Utag link event with payload: "+JSON.stringify(g))
}window.utag.link(g)
})
}})
});
(function(){window.HMTrackingManager=function(){var publ=this;
var priv={debugTracking:false,eventType:{ADD_FAVORITE:"add_favorite",REMOVE_FAVORITE:"remove_favorite",FAQ_OPENING:"faq_opening",POPUP:"popup",BANNER:"club_banner"},colorArticleCode:""};
...
if(jsonParamsString){jsonParams=JSON.parse(jsonParamsString.trim())
